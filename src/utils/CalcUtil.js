...
async function calcFileSha1({file, file_path, pre_size, process_calc_sha1_size, onProgress, getStopFlagFun, context}) {
  if (context.isNode) {
    // 桌面端
...
async function calcFileCrc64({file, file_path, onProgress, getStopFlagFun, context, process_calc_crc64_size}) {
  if (context.isNode) {
    file_path = file_path || file.path
    let size = context.fs.statSync(file_path).size
