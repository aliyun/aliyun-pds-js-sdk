...
function get_available_size(
  ...
  // 找最近的 64x
  chunk_size = find_64x(chunk_size)
...
// 找最近的 64x
function find_64x(chunk_size) {
  return chunk_size + (64 - (chunk_size % 64))
}
...
function init_chunks_sha(
  ...
  let part_info_list: IUpPartInfo[] = []
...
function init_chunks_download(file_size: number, init_chunk_size: number = INIT_CHUNK_SIZE): [IDownPartInfo[], number] {
  ...
  let part_info_list: IDownPartInfo[] = []
  let i = 0
  for (i = 0; i < num; i++) {
    part_info_list.push({
      part_number: 1 + i,
...
function init_chunks_web_download(file_size: number): [IDownPartInfo[], number] {
  return [
    [
      {
        part_number: 1,
