import {ready as sha1Ready, sha1 as calc_sha1, createSha1} from '../sha1/js-sha1-origin'
import {ready as sha256Ready, sha256 as calc_sha256, createSha256} from '../sha256/js-sha256-origin'
...
export {js_calc_hash, js_calc_file_hash, js_calc_file_hash_parts}

async function js_calc_hash(algorithm, buf) {
  return algorithm == 'sha1' ? await calc_sha1(buf) : await calc_sha256(buf)
}

async function js_calc_file_hash_parts(type, file, parts, onProgress, getStopFlag) {
...
// 使用 js 计算
async function js_calc_file_hash(type, file, preSize, onProgress, getStopFlag) {
  type == 'sha1' ? await sha1Ready() : await sha256Ready()
