...
function checkWasmReady() {
  // WASM 初始化完成的标志：calledRun 为 true 且 asm 对象存在
  return binding.calledRun && binding.asm && typeof binding.asm.C === 'function'
}

/* istanbul ignore next */
async function ready() {
  if (isReady || checkWasmReady()) {
    isReady = true
    return
  }
...
// 保存原始的 onRuntimeInitialized（如果有）
const originalOnInit = binding.onRuntimeInitialized
...
binding.onRuntimeInitialized = function () {
  ...
  // 调用原始的回调（如果存在）
  if (typeof originalOnInit === 'function' && originalOnInit !== binding.onRuntimeInitialized) {
    originalOnInit()
  }
}

// 如果 WASM 已经初始化完成，立即设置 isReady
if (checkWasmReady()) {
  isReady = true
}
