...
import {ready as sha1Ready, createSha1 as wasmCreateSha1} from '../utils/sha1/wasm/index'
import {ready as sha256Ready, createSha256 as wasmCreateSha256} from '../utils/sha256/wasm/index'
...
function calc_sha1(str, context) {
  const {crypto} = context || NodeContext
  return crypto.createHash('sha1').update(Buffer.from(str)).digest('hex').toUpperCase()
}
function calc_sha256(str, context) {
  const {crypto} = context || NodeContext
  return crypto.createHash('sha256').update(Buffer.from(str)).digest('hex').toUpperCase()
}

function calc_crc64(str, last) {
  return crc64(Buffer.from(str), last)
}
...
/// for node.js
async function calc_file_sha1(file_path, size, onProgress, getStopFlag, context) {
  return await _calc_file_sha('sha1', file_path, size, onProgress, getStopFlag, context)
}
async function calc_file_sha256(file_path, size, onProgress, getStopFlag, context) {
  return await _calc_file_sha('sha256', file_path, size, onProgress, getStopFlag, context)
}
async function _calc_file_sha(type, file_path, size, onProgress, getStopFlag, context) {
  // await ready()
...
async function calc_file_parts_sha1(file_path, parts, onProgress, getStopFlag, context) {
  return await _calc_file_parts_sha('sha1', file_path, parts, onProgress, getStopFlag, context)
}
async function calc_file_parts_sha256(file_path, parts, onProgress, getStopFlag, context) {
  return await _calc_file_parts_sha('sha256', file_path, parts, onProgress, getStopFlag, context)
}
async function _calc_file_parts_sha(type, file_path, parts, onProgress, getStopFlag, context) {
  ...
  let hash = type == 'sha1' ? wasmCreateSha1() : wasmCreateSha256()
