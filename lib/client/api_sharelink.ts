...
export interface IGetShareTokenReq {
  share_id: string
  share_pwd?: string
  expire_sec?: number // 令牌有效时长。参数合法范围是(0, 7200]；参数为0或缺省时默认7200秒。
  check_share_pwd?: boolean // 管理员可以免密访问
}
...
export interface ICreateShareLinkReq {
  drive_id: string
  share_all_files?: boolean // 是否分享整个drive中的文件
  file_id_list?: string[] // 分享父路径文件id列表。 父路径文件个数范围[1, 100]。 如果share_all_files=true，那么此字段无效，否则必须要填写
  ...
  preview_limit?: number // 分享预览次数限制，0表示不限制
  save_limit?: number
  download_limit?: number // 分享下载次数限制。 次数0表示不限制
...
export interface IAnonymousShareLinkItem {
  ...
  creator_name?: string //创建者名字（已脱敏）
  creator_phone?: string //创建者手机号（已脱敏）
  comments?: string
