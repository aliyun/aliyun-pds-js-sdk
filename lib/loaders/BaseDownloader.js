...
import {calcDownloadMaxConcurrency, calcSmoothSpeed} from '../utils/LoadUtil'
...
export class BaseDownloader extends BaseLoader {
  ...
  async start() {
    ...
    if (!['waiting', 'prepare', 'error', 'stopped', 'cancelled'].includes(this.state)) return
  ...
  async run() {
    ...
    if (this.archive_file_ids?.length) await this.prepare()
    // this.download_url
    // this.file.size
