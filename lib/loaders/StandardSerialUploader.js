...
export class StandardSerialUploader extends BaseUploader {
  ...
  initChunks(parts = []) {
    ...
    debug('initChunks result:', part_info_list)
  ...
  async calcSha1(file, pre_size, onProgress = () => {}, getStopFlag = () => {}) {
    this.timeLogStart(pre_size ? 'pre_sha1' : 'sha1', Date.now())

    let _sha1_fun = this.custom_sha1_fun || this.context_ext.calcFileSha1
    let result = await _sha1_fun.call(this.context_ext, {
      file,
