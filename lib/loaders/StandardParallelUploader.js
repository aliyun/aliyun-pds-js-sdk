...
export class StandardParallelUploader extends ParallelUploader {
  ...
  async calcHash(file, onProgress = () => {}, getStopFlag = () => {}) {
    this.timeLogStart('multi_sha1', Date.now())

    let _parts_sha1_fun = this.custom_parts_sha1_fun || this.context_ext.calcFilePartsSha1

    let {part_info_list, content_hash} = await _parts_sha1_fun.call(this.context_ext, {
      file,
